  ...
  /**
   * Pulls the first image payload from an Imagen `:predict` JSON body (shape varies slightly by API revision).
   * @param {unknown} body
  ...
  Given this headline: "${headline}", create one absurd obstacle for a Japanese game show in 1-2 sentences.`
      ...
      /**
       * Synthesizes a unique visual context for the dynamic obstacle using Imagen, enhancing player immersion.
       * Imagen is reached via the Generative Language `:predict` endpoint; `@google/generative-ai` does not yet surface `generateImages`, so the same API key backs this REST hop and the SDK text calls.
       * The PNG bytes are wrapped in a data URI so the browser can consume `image_data` as an `img` src without a second fetch.
       */
      let image_data = null;
